...
public class ServerController {
    ...
    public ResponseEntity<List<DataEnvelope>> getDataByBlockType(@PathVariable String blockType) throws IOException {
        Objects.requireNonNull(blockType);
        log.info("Request received to query data for block type {}", blockType);
        List<DataEnvelope> data = server.findBy(BlockTypeEnum.valueOf(blockType));
        return ResponseEntity.ok(data);
    }


    @PatchMapping(value = "/update/{name}/{newBlockType}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Boolean> updateBlockType(@PathVariable String name, @PathVariable String newBlockType) throws IOException {
        ...
        boolean result = server.updateBlockType(name, newBlockType);
